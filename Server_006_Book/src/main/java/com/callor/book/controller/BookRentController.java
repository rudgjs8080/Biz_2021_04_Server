...
@WebServlet("/rent/*") // rent로 시작되는 모든 명령을 수행해라
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath.equals("/seq")) {
			...
			if (strSeq == null || strSeq.equals("")) {
				...
				Long nSeq = Long.valueOf(strSeq, 10);// ,10 지워야함
				BookRentDTO brDTO = brService.findById(nSeq);

				// view에서 보여줄 데이터 생성
				/*
				 * ServletContext app Tomcat을 기반으로 작성된 Web App Service에서 요청(Req) 응답(Res)를 총괄하는
				 * 정보가 담긴 객체 Web App Service를 구현하기 위하여 Req, Res 를 처리하는 여러가지 기능을 구현해야 하는데 그러한 기능을
				 * 미리 구현해 놓았기 때문에 ServletContext를 getter 하는 것만으로 충분하다
				 * 
				 * DB 등으로부터 조회된 데이터를 Web에게 응답하고자 할 때 쉬운 방법으로 전달할 수 있도록 하는 기능이 미리 구현되어 있다
				 */
				...
				// order_info.jsp 파일을 읽어서
				// app에 setting 한 BOOK 변수와 함께
				// Rendering을 하라
				// wepapp/WEB-INF/views/order_info.jsp 파일을 읽어서
			...
			if (buyerDTO != null) {
				...
				// bu_code값에 해당하는 회원정보가 있으면 Console에 출력
			}
			// ServletContext app = req.getServletContext();

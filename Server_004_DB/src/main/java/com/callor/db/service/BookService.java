...
public interface BookService {
	 ...
	 * Controller에서 Service의 select() method를 호출 할 것이다
	 * 
	 * select() method는
	...
	// 저자코드로 검색
	public List<BookVO> findByAcode(String bk_acode);
	// 발행일로 검색
	public List<BookVO> findByDate(String bk_date);
	// 가격으로 검색
	public List<BookVO> findByPrice(Integer bk_price);
	/*
	 * 변경하고자 하는 데이터를 전달받아서
	 ...
	 * 데이터를 UPDATE
	 */
	public void update(BookVO bookVO); // update, 데이터 갱신
	
	// 삭제하고자 하는 데이터의 PK값으 매개변수로 전달받고
	// 1개의 데이터를 삭제한다
	public void delete(String bookVO); // delete, 데이터 삭제 
